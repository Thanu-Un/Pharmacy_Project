...
export default function AddSupplier({ onBack, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/operation/suppliers', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorMsg || 'Failed to create supplier');
      }
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Register a new wholesale supplier or distributor</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input 
              ...
              onChange={handleChange} 
              className="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none" 
              placeholder="Enter city (e.g. Phnom Penh)" 
            />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
          ...
          <button 
            type="submit" 
            disabled={isLoading} 
            className="px-6 py-2.5 rounded-lg bg-emerald-500 text-white font-medium hover:bg-emerald-600 transition-colors disabled:opacity-50 flex items-center gap-2 shadow-sm"
          >
