...
export default function ProductsReport() {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      let url = '/api/reporting/products-report';
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-64 text-slate-500">Loading product report...</div>;
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-emerald-600 text-white text-sm">
              ...
              <th className="p-3 font-medium border-b border-emerald-700 whitespace-nowrap">Product Name</th>
              ...
              <th className="p-3 font-medium border-b border-emerald-700 text-right whitespace-nowrap">Stock (Qty) Amt</th>
          ...
          <tbody className="text-sm">
            {reportData.map((row, index) => (
              <tr key={index} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                ...
                <td className="p-3 text-right">
                  <span className="text-slate-500 mr-2">({row.purchasedQty || 0})</span>
                  <span className="font-medium text-slate-800">{(row.purchasedAmt || 0).toFixed(2)}$</span>
                </td>
                <td className="p-3 text-right">
                  <span className="text-slate-500 mr-2">({row.soldQty || 0})</span>
                  <span className="font-medium text-slate-800">{(row.soldAmt || 0).toFixed(2)}$</span>
                </td>
                <td className="p-3 text-right">
                  <span className="text-slate-500 mr-2">({row.stockQty || 0})</span>
                  <span className="font-medium text-slate-800">{(row.stockAmt || 0).toFixed(2)}$</span>
                </td>
                <td className="p-3 text-right">
            ...
            <tr className="bg-slate-50 border-t-2 border-slate-300 font-semibold text-slate-800">
              ...
              <td className="p-3 text-right">{totals.stockAmt.toFixed(2)}$</td>
        ...
        {reportData.length === 0 && (
          <div className="p-8 text-center text-slate-500">
            No product data available for the report.
          </div>
        )}
      </div>
    </div>
