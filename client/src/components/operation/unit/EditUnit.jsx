...
export default function EditUnit({ unit, onSaveSuccess, onCancel }) {
    ...
    fetch('/api/operation/units')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          setUnits(data.filter(u => u.id !== unit?.id));
        }
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/operation/units/${unit.id}`, {
        method: 'PUT',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorMsg || 'Failed to update unit');
      }
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Update existing unit of measurement</p>
        </div>
        <button 
          onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8">
          ...
          <div className="space-y-6">
            <div>
              ...
              />
              <p className="text-xs text-slate-400 mt-1">Unit code cannot be changed.</p>
            </div>
