...
export default function EditProduct({ product, onBack, onSave }) {
  ...
  const [formData, setFormData] = useState({
    ...
    baseUnit1: product?.baseUnit1 || null,
    priceBaseUnit1: product?.priceBaseUnit1 || '',
    
    baseUnit2: product?.baseUnit2 || null,
    priceBaseUnit2: product?.priceBaseUnit2 || '',
    
    baseUnit3: product?.baseUnit3 || null,
    priceBaseUnit3: product?.priceBaseUnit3 || '',
    
    baseUnit4: product?.baseUnit4 || null,
    priceBaseUnit4: product?.priceBaseUnit4 || '',
    
    baseUnit5: product?.baseUnit5 || null,
    priceBaseUnit5: product?.priceBaseUnit5 || '',
  ...
  useEffect(() => {
    ...
    if (formData.unit) {
      ...
      setFormData(prev => {
        const updated = { ...prev };
        // Reset all base units and their prices first
        for (let i = 1; i <= 5; i++) {
          updated[`baseUnit${i}`] = null;
          updated[`priceBaseUnit${i}`] = '';
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/operation/products/${product.id}`, {
        method: 'PUT',
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <button type="button" onClick={onBack} className="p-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-600 transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
        </button>
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Edit Medicine</h1>
          <p className="text-sm text-slate-500 mt-1">Update details for {product?.name}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select required value={formData.category?.id || ''} onChange={(e) => handleSelectChange('category', e.target.value, categories)} className="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none bg-white">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="max-w-md mb-6">
            ...
            <select value={formData.unit?.id || ''} onChange={(e) => handleSelectChange('unit', e.target.value, units)} className="w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
